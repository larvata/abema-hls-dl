...
const downloadTSFilePromise = (url, duration) => {
  let startTime = duration ? new Date().getTime() : null;
  ...
  return new Promise((resolve, reject) => {
    ...
    .buffer(true)
    .then((resp) => {
      let waitNaturallyTimeout = 0;
      if (duration) {
        const spend = new Date().getTime() - startTime;
        if (spend < duration) {
          waitNaturallyTimeout = duration - spend;
        }
      }
...
const downloadAndDecryptPromise = (fstream, options) => {
  ...
  return decodeAbemaURIPromise(keyUri, session.profile.userId).then((keyBuffer) => {
    ...
    return downloadTSFilePromise(segment.uri, duration).then((tsBuffer)=>{
...
const dumpStreamPromise = (channelId, scheduledTimeEnd, recordingKey) => {
  if (!shouldContinueRecord(scheduledTimeEnd)) {
    ...
    fs.writeFileSync(`${TS_DUMP_CACHE_PATH}/${recordingKey}/playlist.json`, JSON.stringify(session.segmentList, null, 2));
  ...
  return new Promise((resolve, reject) => {
    downloadInitialPlaylistPromise(channelId)
      ...
      .then(() => {
        return dumpStreamPromise(channelId, scheduledTimeEnd, recordingKey);
      })
      .then(()=>{
        return resolve();
      })
