...
const downloadInitialPlaylistPromise = (channelId, resolution) => {
  ...
  const request = session.agent.get(pl.playlist)
    ...
    .then((plOfpl) => {
      ...
      // get playlist by resolution value, if failed use the best resultion
      const matched = plOfpl.playlists.reduce((a, b) => {
        if (!a) {
          return b;
        }
        ...
        if (resolution) {
          // determine playlist by resolution
          const urlParts = b.uri.split('/');
          const px = urlParts[0];
      ...
      const url = `${pl.baseUrl}/${matched.uri}`;
    ...
    .catch((e) => {
      ...
      return setTimeoutPromise(RETRY_INTERVAL)
        .then(()=> downloadInitialPlaylistPromise(channelId, resolution))
        .then((pl) => {
          return Promise.resolve(pl);
      });
    });
...
const dumpStreamPromise = (channelId, recordingKey, resolution) => {
  ...
  return new Promise((resolve, reject) => {
    downloadInitialPlaylistPromise(channelId, resolution)
      .then((playlist) => {
      ...
      .then(() => {
        return dumpStreamPromise(channelId, recordingKey, resolution);
      })
      .then(()=>{
        return resolve();
      })
...
const scheduleDumpPromise = (options) => {
    ...
    const defaultFileName = `${recordingKey}.ts`;
    const recordFileName = outputFileName || defaultFileName;
    const tsFileStream = fs.createWriteStream(`./${TS_DOWNLOAD_PATH}/${recordFileName}`);
